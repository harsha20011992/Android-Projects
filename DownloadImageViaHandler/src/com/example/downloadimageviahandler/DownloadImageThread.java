package com.example.downloadimageviahandler;
...
public class DownloadImageThread extends Thread{
	...
	public DownloadImageThread(String UrlToDownload1, Activity mActivity1,Handler mainThreadHandler1){
		try {
		...
		mainThreadHandler = mainThreadHandler1;
		...
		public void run() {
			...
			mainThreadHandler.post(new Runnable() {
				...
				public void run() {
					...
					test1 = (MainActivity) mActivity;
					test1.LoadingSection.setVisibility(View.VISIBLE);
					Log.d("harsha", Thread.currentThread().getName());
				}
			});
			//Toast.makeText(test1, "DownloadStarted",Toast.LENGTH_SHORT).show();
			DownloadImageCode();
		}
		...
		public void DownloadImageCode(){
			if(urlToDownload!=null){
				try {
					...
					// TODO Auto-generated catch block
					e.printStackTrace();
					//Toast.makeText(test1, "some exception",Toast.LENGTH_SHORT).show();
					...
					mainThreadHandler.post(new Runnable() {
						...
						public void run() {
							...
							//Toast.makeText(test1, "Successfully downloaded",Toast.LENGTH_SHORT).show();
						}
					});
